import processing.serial.*; //import all libraries
import java.awt.event.KeyEvent; // imports library for reading the data from the serial port
import java.io.IOException;
  
Serial myPort; //defines Serial Port
  
String angle="";// defines variables
String distance="";
String data="";
String noObject;
float pixsDistance;
...
void setup() {
 size (1366, 700);
 smooth();
 myPort = new Serial(this,"COM5", 9600);// starts the serial communication vie COM5 port
 myPort.bufferUntil('.'); 
}
...
void draw() {
  ...
  rect(0, 0, width, 1010); 
  fill(98,245,31);
  drawRadar(); // calls fns for drawing the radar
  drawLine();
  drawObject();
  drawText();
}
...
void drawRadar() {
  pushMatrix();
  translate(683,700);  // moves the starting coordinats to new location
  noFill();
  strokeWeight(2);
  stroke(98,245,31);
  // draws the arc lines
